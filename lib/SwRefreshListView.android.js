...
//----------------------------------------------
  ...
  listViewHeight(e) {
    ...
    if (this.state.androidBom>0) {
      return;
    }
    const onLayoutHeight = e.layout.height;
    this.setState({
      isShowHeader: true,
    }, () => {
      setTimeout(()=>{
        this.refs.listView.scrollTo({x: 0, y: 70, animated: true})
      }, 100)
      let androidBom = containerHeight - onLayoutHeight;
      console.log(androidBom)
      if (androidBom === 40) {
        androidBom = 0;
      }
      if (androidBom>0) {
        this.setState({
          androidBom: androidBom,
        }, () => {
          setTimeout(()=>{
            this.refs.listView.scrollTo({x: 0, y: 70, animated: true})
          }, 100)
  ...
  render(){
    ...
    // console.log(this.state.androidBom)
    // console.log(this.state.onLayoutHeight)
    return(
      <View>
      <ListView
        ref="listView"
        onLayout={({ nativeEvent: e }) => { this.listViewHeight(e); }}
        contentContainerStyle={{ paddingBottom: this.state.androidBom || 0}}
